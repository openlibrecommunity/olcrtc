...
func (m *Multiplexer) SendData(sid uint16, data []byte) error {
	...
	for i := 0; i < len(data); i += chunkSize {
		...
		binary.BigEndian.PutUint16(frame[6:8], uint16(chunkLen))
...
func (m *Multiplexer) HandleFrame(frame []byte) {
	...
	if sid == 0xFFFF && length == 0xFFFF {
		m.handleGlobalReset(clientID)
		return
	}

	if length == 0 {
		m.handleStreamClose(sid, clientID)
		return
	}
	...
	m.handleStreamData(sid, clientID, seq, data)
}

func (m *Multiplexer) handleResetFrame(frame []byte) {
	if len(frame) < 8 {
		return
	}
...
func (m *Multiplexer) handleGlobalReset(clientID uint32) {
	m.mu.Lock()
...
func (m *Multiplexer) handleStreamClose(sid uint16, clientID uint32) {
	m.mu.Lock()
...
func (m *Multiplexer) handleStreamData(sid uint16, clientID uint32, seq uint32, data []byte) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	m.processStreamData(stream, seq, data)
}

func (m *Multiplexer) processStreamData(stream *Stream, seq uint32, data []byte) {
	if seq == stream.nextSeq {
		...
		m.processOutOfOrderQueue(stream)
		m.notifyDataReady(stream.ID)
...
func (m *Multiplexer) processOutOfOrderQueue(stream *Stream) {
	for {
